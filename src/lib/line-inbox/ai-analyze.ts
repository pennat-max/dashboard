...
function buildPrompt(rawText: string): string {
  return [
    ...
    "- Preserve all numbers, units, percentages, quantities, dates, model names, brand names, and reference words from actual work items.",
    "- Do not shorten actionable lines. Example: 'กรอไมล์ 77,000 km.' must stay exactly as one work item, not 'กรอไมล์'.",
    "- Deduplicate repeated work items inside the same message. If a short and long version describe the same work, keep only the longer/more complete version.",
    "- If duplicate versions differ only by 'ตามรูป' or 'ตามภาพ', keep the version with 'ตามรูป' or 'ตามภาพ'.",
