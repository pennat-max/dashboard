...
import { carTitleLine, isWebsitePendingBeForward } from "@/lib/car-fields";
...
export default async function WebsitePendingBeForwardPage() {
  ...
  const p = dict.websitePendingBeForwardPage;
  const c = dict.common;
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="min-w-[120px]">{p.colBuyer}</TableHead>
              <TableHead className="min-w-[140px]">{p.colBfOnWeb}</TableHead>
              ...
              sortedRows.map((car) => {
                ...
                return (
                  <TableRow key={String(car.row_id ?? car.id)}>
                    ...
                    <TableCell className="text-muted-foreground">{(car.buyer ?? "").trim() || c.buyerUnknown}</TableCell>
                    <TableCell className="text-muted-foreground">{(car.bf_on_web ?? "").trim() || c.dash}</TableCell>
