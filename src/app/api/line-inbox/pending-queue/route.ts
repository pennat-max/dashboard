...
import { publicUrlForLineInboxStoragePath } from "@/lib/line-inbox/upload-line-inbox-image";
...
export async function GET() {
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .select("id,received_at,raw_text,analyze_payload,car_row_id,image_storage_path")
    ...
    for (const row of data ?? []) {
      ...
      const imgPath = String((row as { image_storage_path?: unknown }).image_storage_path ?? "").trim();
      const image_public_url =
        imgPath && !imgPath.includes("..") ? publicUrlForLineInboxStoragePath(supabase, imgPath) : null;
      ...
      messages.push({
        ...
        image_public_url: image_public_url || null,
      });
    }
