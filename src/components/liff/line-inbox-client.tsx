...
export function LineInboxClient({ initialCarRowId = "", initialCarId = null }: Props) {
  ...
  const [taskLinesSource, setTaskLinesSource] = useState<LineInboxAnalyzeResponse["task_lines_source"]>(
    undefined
  );
    ...
    <div className="flex min-h-0 flex-col gap-4 bg-white px-3 py-4 text-sm text-slate-900">
      ...
      <section className="space-y-2 rounded-lg border border-slate-200 bg-slate-50/80 p-3">
        ...
        <div className="flex flex-wrap items-stretch gap-2">
          ...
          {!analyzeLoading && (taskLinesHeuristic.length > 0 || rows.length > 0) ? (
      ...
      {detected ? (
        <section className="space-y-2 rounded-lg border border-slate-200 p-3">
          ...
          <dl className="grid grid-cols-2 gap-x-2 gap-y-1 text-[12px]">
            ...
            <dt className="text-slate-500">สเปก (แสดงผลรวม)</dt>
            <dd className="break-words text-[11px] font-medium">{detected.spec?.trim() || "—"}</dd>
            <dt className="text-slate-500">สเปกจากข้อความ (เทียบ)</dt>
            <dd className="break-words text-[11px] text-slate-800">
              {detected.line_spec_snippet?.trim() || "—"}
            </dd>
            <dt className="text-slate-500">สเปกใน DB</dt>
            <dd className="break-words text-[11px] text-slate-700">
              {detected.db_spec?.trim() || "—"}
            </dd>
