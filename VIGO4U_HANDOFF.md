...
## Latest Update
- **[LINEBridge - analyze quality bugfix - May 2026]** LINE Inbox analyze now de-duplicates same-message suggested work items after AI + rule guards. When a short item and a fuller item describe the same work, the fuller line wins; lines with `ตามรูป` / `ตามภาพ` win over versions without the reference text; and numeric/unit details such as `77,000 km.`, percentages, dates, quantities, and model/brand text are preserved in `suggested_item_name`. `กรอไมล์ 77,000 km.` is treated as a real work item, while vehicle spec lines, people/mention context, and noise remain ignored context. Analyze/analyze-pending remain advisory only and do **not** create `order_items`, do **not** auto-save, and do **not** change Supabase schema or `public.cars`.
