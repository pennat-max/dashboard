...
## Latest Update
- **[OrderTracking chip-cache experiment - May 2026]** Added a real `/m/orders` experiment behind `NEXT_PUBLIC_ORDER_CHIP_CACHE_ENABLED=false` (default off). When off, the old `/m/orders` flow remains active. When on, `/m/orders` loads the full car index for search/filter/count relationships, hydrates only the first 50 matching card details, caches loaded card details client-side, and loads the next 50 through `POST /api/m/order-tracking/card-details` as the user scrolls. Chip counts are recomputed from the active filter context; item/staff filters use a lightweight `order_items` filter index, while card rows hydrate details separately. No `public.cars` schema change, no migration, no Google Sheet Sync, no LINE Bot/LIFF change.
