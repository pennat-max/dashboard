...
## Latest Update
- **[LINEBridge manual confirmation - May 2026]** Improved the manual LINE Inbox confirmation UI in `/m/orders` (`LineInboxAiToolbar`) and `/liff/line-inbox`: analyze still stays read-only, but suggested rows now default to saving only `new` lines while duplicate / possible duplicate / unclear rows start as skipped. Staff can bulk-select new/all/none, edit the suggested item name/status/note, and choose skip/create/merge before calling the existing confirm API. No webhook change, no schema change, no automatic LINE reply.
