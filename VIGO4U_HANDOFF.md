...
## Latest Update
- **[LINEBridge Phase 1 manual inbox - May 2026]** Improved the manual LINE Inbox workflow in `/m/orders` only (`LineInboxAiToolbar`): clearer `LINE Inbox` section and `วางข้อความจาก LINE` paste area, car search/select before saving, detected car/chassis display, duplicate/new/possible duplicate review rows, editable item name/status/note, selected-line save only, duplicate warning before save, pending queue selection retained, and copy-ready LINE reply text after save. No webhook change, no LIFF implementation change, no automatic reply, no Supabase schema change.
