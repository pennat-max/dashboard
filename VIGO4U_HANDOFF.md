...
## Latest Update
- **[LINEBridge — Issue #20 car matching/spec filter — May 2026]** Updated the LINE Inbox analyze pipeline so car identity/spec lines are separated from saveable work items. Matching now prefers explicit `car_row_id`, exact chassis, then normalized plate matches; ambiguous matches return candidate info and require human review. Vehicle model/spec-only lines (model, drivetrain, engine, cabin, color, year, chassis/plate-only, price/mileage-only) are returned as `ignored_vehicle_spec_lines` for context and shown in `/m/orders` as “ใช้สำหรับจับรถ ไม่ใช่งาน”. No LINE webhook change, no LIFF change, no Supabase schema change, and no `public.cars` change.
