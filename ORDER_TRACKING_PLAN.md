...
- **LINE Inbox + AI:** **`POST /api/line-inbox/analyze`** (read-only suggestions) + **`POST /api/line-inbox/confirm`** (writes after human confirm) + LIFF **`/liff/line-inbox`**. Spec: **`LINE_INBOX_AI_ANALYSIS_PLAN.md`**. **No** auto-save from analyze; **no** secrets in browser. `/m/orders` Phase 1 manual inbox keeps human review first: staff paste LINE text, analyze, select/search car if needed, edit/tick suggested lines, confirm save, then copy a ready LINE reply manually.
