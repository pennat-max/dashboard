...
- **LINE Inbox detail grouping:** main action lines plus following detail/spec lines should become one proposed `order_item`; detail rows go into note. Example: `ติดฟิล์ม รอบคัน` with `ประตู 80%` / `กระจกบานหน้า 60%` becomes one item `ติดฟิล์มรอบคัน` and note `ประตู 80% / กระจกบานหน้า 60%`.
