...
- **LINE Inbox analyze quality:** after AI and deterministic guards, same-message duplicate suggestions are collapsed. If a short and full version both exist, keep the fuller version; if one version includes `ตามรูป` / `ตามภาพ`, keep that version; always preserve numbers, units, percentages, quantities, dates, and model/brand details in the saveable suggested item text. AI/duplicate reason strings are staff-hidden by default and should only appear in dev/debug details.
...
- **LINE pending analyze (Issue #34):** `POST /api/line-inbox/analyze-pending` analyzes captured pending inbox rows outside the webhook request, writes analyze results back to `line_inbox_messages`, and keeps `order_items` writes behind the existing human-confirm save flow.
- **LINE image capture (Issue #38):** LINE image/image-file events are capture-only. The webhook may download image content with `LINE_CHANNEL_ACCESS_TOKEN`, store it in the existing `order-tracking-photos` storage path, and expose recent LINE photos to the human review UI for `ตามรูป` / `ตามภาพ` items. Images are attached to `order_items` only after staff approves/saves the suggested item. No auto reply, no auto-save, no new schema migration, and no `public.cars` change.
- **LINE action queue (Issue #40):** `/m/orders` LINE Inbox uses existing analyzed `line_inbox_messages` rows as a staff `รอจัดการ` queue. Queue cards group messages by detected car, show source text/photos/existing tasks, and allow create/merge/skip per suggested action before any `order_items` write. No auto reply, no auto-save, no `line_action_queue` table yet, no schema migration, and no `public.cars` change.
